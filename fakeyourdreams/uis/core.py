...
from fakeyourdreams.uis import overrides
from fakeyourdreams import metadata, logger, wording
from fakeyourdreams.uis.typing import Component, ComponentName
from fakeyourdreams.filesystem import resolve_relative_path
...
def load_ui_layout_module(ui_layout : str) -> Any:
	try:
		ui_layout_module = importlib.import_module('fakeyourdreams.uis.layouts.' + ui_layout)
...
def launch() -> None:
	ui_layouts_total = len(fakeyourdreams.globals.ui_layouts)
	with gradio.Blocks(theme = get_theme(), css = get_css(), title = metadata.get('name') + ' ' + metadata.get('version')) as ui:
		for ui_layout in fakeyourdreams.globals.ui_layouts:
			ui_layout_module = load_ui_layout_module(ui_layout)
