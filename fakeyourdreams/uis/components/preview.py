...
from fakeyourdreams import logger, wording
from fakeyourdreams.audio import get_audio_frame, create_empty_audio_frame
from fakeyourdreams.common_helper import get_first
from fakeyourdreams.core import conditional_append_reference_faces
from fakeyourdreams.face_analyser import get_average_face, clear_face_analyser
from fakeyourdreams.face_store import clear_static_faces, get_reference_faces, clear_reference_faces
from fakeyourdreams.typing import Face, FaceSet, AudioFrame, VisionFrame
from fakeyourdreams.vision import get_video_frame, count_video_frame_total, normalize_frame_color, resize_frame_resolution, read_static_image, read_static_images
from fakeyourdreams.filesystem import is_image, is_video, filter_audio_paths
from fakeyourdreams.content_analyser import analyse_frame
from fakeyourdreams.processors.frame.core import load_frame_processor_module
from fakeyourdreams.uis.core import get_ui_component, get_ui_components, register_ui_component
...
def render() -> None:
	...
	if source_audio_path and fakeyourdreams.globals.output_video_fps and fakeyourdreams.globals.reference_frame_number:
		temp_audio_frame = get_audio_frame(source_audio_path, fakeyourdreams.globals.output_video_fps, fakeyourdreams.globals.reference_frame_number)
		if numpy.any(temp_audio_frame):
			source_audio_frame = temp_audio_frame
	if is_image(fakeyourdreams.globals.target_path):
		target_vision_frame = read_static_image(fakeyourdreams.globals.target_path)
	...
	if is_video(fakeyourdreams.globals.target_path):
		temp_vision_frame = get_video_frame(fakeyourdreams.globals.target_path, fakeyourdreams.globals.reference_frame_number)
		preview_vision_frame = process_preview_frame(reference_faces, source_face, source_audio_frame, temp_vision_frame)
		preview_image_args['value'] = normalize_frame_color(preview_vision_frame)
		preview_image_args['visible'] = True
		preview_frame_slider_args['value'] = fakeyourdreams.globals.reference_frame_number
		preview_frame_slider_args['maximum'] = count_video_frame_total(fakeyourdreams.globals.target_path)
...
def update_preview_image(frame_number : int = 0) -> gradio.Image:
	for frame_processor in fakeyourdreams.globals.frame_processors:
	...
	source_audio_frame = create_empty_audio_frame()
	if source_audio_path and fakeyourdreams.globals.output_video_fps and fakeyourdreams.globals.reference_frame_number:
		reference_audio_frame_number = fakeyourdreams.globals.reference_frame_number
		if fakeyourdreams.globals.trim_frame_start:
			reference_audio_frame_number -= fakeyourdreams.globals.trim_frame_start
		temp_audio_frame = get_audio_frame(source_audio_path, fakeyourdreams.globals.output_video_fps, reference_audio_frame_number)
		if numpy.any(temp_audio_frame):
			source_audio_frame = temp_audio_frame
	if is_image(fakeyourdreams.globals.target_path):
		target_vision_frame = read_static_image(fakeyourdreams.globals.target_path)
	...
	if is_video(fakeyourdreams.globals.target_path):
		temp_vision_frame = get_video_frame(fakeyourdreams.globals.target_path, frame_number)
		preview_vision_frame = process_preview_frame(reference_faces, source_face, source_audio_frame, temp_vision_frame)
		preview_vision_frame = normalize_frame_color(preview_vision_frame)
		return gradio.Image(value = preview_vision_frame)
